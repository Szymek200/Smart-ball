...
static void gps_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    switch (event_id) {
        case GPS_UPDATE:
            // Standardowy handler (zostawiamy pusty, bo parser odrzuca prefiks $GN)
            break;

        case GPS_UNKNOWN: {
            // Bezpiecznie wyciągamy surowy tekst, który przysyła Twój moduł
            char *unknown_runtime_buffer = (char *)event_data;
            if (unknown_runtime_buffer != NULL) {
                ESP_LOGW("GPS_UNKNOWN_STREAM", "Odebrano: %s", unknown_runtime_buffer);
            }
            break;
        }
        default:
...
static void gps_read_task(void *pvParameters)
{
    ...
    while (1) {
        // Czytamy jeden bajt z UART-u (blokuje zadanie dopóki coś nie przyleci)
        int len = uart_read_bytes(GPS_UART_NUM, data, 1, pdMS_TO_TICKS(1000));
        
        if (len > 0) {
            char c = data[0];

            // Jeśli to nie jest koniec linii, zbieramy znaki do bufora
            if (c != '\r' && c != '\n') {
                if (line_idx < sizeof(line_buffer) - 1) {
                    line_buffer[line_idx++] = c;
                }
            } 
            // Jeśli wykryliśmy koniec linii (\n) i bufor nie jest pusty
            else if (c == '\n' && line_idx > 0) {
                line_buffer[line_idx] = '\0'; // Zamknięcie stringa

                // Tutaj masz czysty, surowy string prosto z satelity!
                ESP_LOGI("GPS_SUROWY_UART", "%s", line_buffer);

                // Może tu stać Twój ręczny sscanf, np.:
                // if (strstr(line_buffer, "RMC")) { ... twój parser ... }

                line_idx = 0; // Reset bufora na kolejną linię
            }
        }
    }
}*/
